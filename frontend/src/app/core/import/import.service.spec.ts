...
describe('ImportService', () => {
  ...
  it('PCD não existe mais como vínculo — é reconhecido como sinônimo de CLT (compatibilidade)', async () => {
    ...
    expect(r.erros).toEqual([]);
    expect(r.grupos[0].linhas).toEqual([{ cbo: '411010', tipo: 'CLT', quantidade: 1 }]);
  });

  it('reconhece os vínculos que não compõem a base, com sinônimos', async () => {
    const r = await servico.importarPlanilha(
      planilha([
        ['CBO', 'TIPO', 'QUANTIDADE'],
        ['411010', 'Afastado pelo INSS', 2],
        ['411010', 'INSS', 1],
        ['514320', 'Trabalhador terceirizado', 3],
        ['514320', 'Trabalho Temporário', 4],
      ]),
    );
    expect(r.erros).toEqual([]);
    expect(r.grupos[0].linhas).toEqual([
      { cbo: '411010', tipo: 'AFASTADO_INSS', quantidade: 2 },
      { cbo: '411010', tipo: 'AFASTADO_INSS', quantidade: 1 },
      { cbo: '514320', tipo: 'TERCEIRIZADO', quantidade: 3 },
      { cbo: '514320', tipo: 'TEMPORARIO', quantidade: 4 },
    ]);
  });

  it('planilha antiga com coluna CARGO_CONFIANCA ainda importa (coluna ignorada)', async () => {
    const r = await servico.importarPlanilha(
      planilha([
    ...
    expect(r.erros).toEqual([]);
    expect(r.grupos[0].linhas).toEqual([{ cbo: '411010', tipo: 'CLT', quantidade: 5 }]);
  });
